...
$(function() {
    ...
    {
        cms_ajax_check_url({name:'alias', value:$('#url_text').val()}, {name:'page_id', value:$('#page_id').html()}, {name:'parent_id', value:$('#parent_dropdown').val()});
    }, 0.5);
    $('#parent_dropdown').change(function()
    {
        var args = [{name:'parent_id', value:$('#parent_dropdown').val()}, {name:'page_id', value:$('#page_id').html()}];
