{if $page->template}
	{foreach from=$page->template->get_page_blocks() key='name' item='block' name='foo'}
		...
		{if $block_obj}
			<h3>{$name}</h3>
			<label>Content Type:</label> {html_options class='content_type_picker' id="block_select_`$smarty.foreach.foo.index`" name="block_type[$name]" options=$content_types selected=$block_obj.type}<br />
