...
class CmsModuleRequest extends CmsObject
{
	...
	public function create_form_start($params = array(), $check_keys = false)
	{
		...
		if (!$use_current_page_as_action)
			$this .= $this->create_start_tag('hidden', array('name' => 'mact', 'value' => $mact), true);
			
		if ($params['return_id'] != '')
		{
			$text .= $this->create_start_tag('hidden', array('name' => $params['id'].'returnid', 'value' => $params['return_id']), true);
			if ($inline)
			{
				$text .= $this->create_start_tag('hidden', array('name' => cms_config()->get('query_var'), 'value' => $params['return_id']), true);
			}
		}
		...
		foreach ($extra_params as $key=>$value)
		{
			if ($key != 'module' && $key != 'action' && $key != 'id')
				$text .= $this->create_start_tag('hidden', array('name' => $params['id'].$key, 'value' => $value), true);
		}
