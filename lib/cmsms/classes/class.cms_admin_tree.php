 ...
 * @license GPL
 **/
class CmsAdminTree extends SilkTree
{
    ...
    static public function fill_from_file()
    {
		$xml = simplexml_load_file(join_path(ROOT_DIR,'lib','cmsms','data','adminmenu.xml'));
		
		$root = self::get_instance()->get_root_node();
		
		foreach ($xml as $node)
		{
			self::create_admin_node($node, $root);
		}
		
		//print_r($root->tree);
    }

	static public function create_admin_node(&$xml, &$node)
	{
		...
		if (isset($xml->title))
		{
			$newnode->title = (string)$xml->title;
			$newnode->english_title = (string)$xml->title;
		}

		if (isset($xml->url))
			$newnode->url = SilkRequest::get_calculated_url_base(true) . '/admin/' . (string)$xml->url;
		...
		if (isset($xml->description))
		{
			if ((string)$xml->description != '')
				$newnode->description = (string)$xml->description;
		}
		...
		if (isset($xml->show_in_menu))
		{
			...
			{
				$newnode->show_in_menu = true; //$admin_theme->has_permission((string)$xml->show_in_menu->permission);
			}
			else
			{
		...
		if (isset($xml->children))
		{
			...
			{
				self::create_admin_node($child, $newnode);
			}
		}
	}
}

class CmsAdminNode extends SilkNode
{
	var $name = '';
	var $title = '';
	var $english_title = '';
